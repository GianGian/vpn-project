...
public class ServerHandshake {
 ...
 /* Session host/port, and the corresponding ServerSocket  */
    ...
    /**
     * Run server handshake protocol on a handshake socket. Here, we simulate
     * the handshake by just creating a new socket with a preassigned port
    ...
    public static void VerifyClientHello(Socket socket, String CA) throws IOException, CertificateException, NoSuchAlgorithmException, NoSuchProviderException, InvalidKeyException, SignatureException {
        ...
        if (HandMessage.getParameter("MessageType").equals("ClientHello")) {
            ...
            if (Clientcertificate.getSubjectDN().toString().contains("CN=client-pf.ik2206.kth.se")) {
                System.out.println("CLIENT CN OK");
                System.out.println(Clientcertificate.getSubjectDN().toString());
            } else {
                socket.close();
                System.out.println("CLIENT CN KO");
                System.out.println(Clientcertificate.getSubjectDN().toString());
            }
            ...
            matcher = pattern.matcher(Clientcertificate.getSubjectDN().toString());
            //to debug
            //matcher = pattern.matcher("EMAILADDRESS=CIAO@kth1se");
            if (matcher.find()) {
                System.out.println("CLIENT mail OK");
            } else {
                System.out.println("CLIENT mail KO");
                socket.close();
            }
